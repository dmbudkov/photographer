    ...
    <div v-if="userInfo" class="page-wrapper">
        ...
        <main class="main">
            ...
            <ul class="vendor-type vendor-type--cabinet vendor-type--in-tabs" ref="vendorTabs" v-if="userInfo.type === 'vendor'">
                ...
                <!--</li>-->
                <li class="vendor-type__item">
                    <nuxt-link to="/cabinet/chat/" class="vendor-type__label vendor-type__label--current">Мои сообщения</nuxt-link>
            ...
            <div class="tab-block tab-block--cabinet tab-block--chat">
                <!--vendor-chats-->
                <div class="tab-block__item">
                    <div class="messages">
                        <div class="messages__item-holder">
                            <nuxt-link :to="{path: `/cabinet/chat/${chat.selfMessage ? chat.user_to : chat.user_id}`}" v-for="chat in $store.state.availableChats" @click="selectDialog($event, chat.selfMessage ? chat.interlocutor : chat.user, chat.selfMessage ? chat.user_to : chat.user_id)" :key="chat.id" class="messages__item chat-item" :class="{'chat-item--unread': !chat.selfMessage && !chat.read}">
                        <div class="chat-item__pic" v-if="chat.selfMessage ? !chat.interlocutor.profile_pic.length : !chat.user.profile_pic.length"></div>
                                <div  v-else class="chat-item__pic" :style="{'background-image': `url(${chat.selfMessage ? chat.interlocutor.profile_pic : chat.user.profile_pic})`}"></div>
                                <div class="chat-item__right">
                                    <div class="chat-item__name" v-if="chat.selfMessage">
                                   {{chat.interlocutor.firstName}} {{chat.interlocutor.lastName}}
                                    </div>
                                    <div class="chat-item__name" v-else>
                                        {{chat.user.firstName}} {{chat.user.lastName}}
                                    </div>
                                    <div class="chat-item__message" v-text="chat.body.trim()">
                                    </div>
                                    <time class="chat-item__time" v-text="prettifyDate(chat.created_at)"></time>
                        ...
                        <div class="messages__dialog">
                           <span class="messages__text"> Выберите диалог</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            selectDialog(event, interlocutor, userTo) {

                this.$store.commit({
                    type: 'setCurrentDialog',
                    userInfo: {name: `${interlocutor.firstName} ${interlocutor.lastName}`, userPic: interlocutor.profile_pic}
                })
                // this.$router.push({path: `/cabinet/chat/${userTo}`})
