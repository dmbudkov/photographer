<template>
    ...
    <ul class="vendor-type">
        <li class="vendor-type__item" v-for="speciality in specialities" :key="speciality.id">
            <input name="vendorSpecType" :id="'speciality' + speciality.id" :value="speciality.id" @change="checkedSpecialityEvent(speciality.id, speciality.code)" class="vendor-type__input" type="radio" v-model="checkedSpeciality"/>
            <label :for="'speciality' + speciality.id" class="vendor-type__label" v-text="speciality.title">
            </label>
        </li>
    </ul>
</template>
